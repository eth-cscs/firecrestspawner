...
class FirecRESTSpawnerBase(Spawner):
    ...
    # all `req_foo` traits will be available as substvars for templated strings
    req_queue = Unicode(
        '',
    ...
    async def start(self):
        ...
        while True:
            ...
            if status == JobStatus.RUNNING:
                ...
                self.log.warning(f"Job {self.job_id} neither "
...
class FirecRESTSpawnerRegexStates(FirecRESTSpawnerBase):
    ...
    def state_ispending(self):
        ...
        return self.job_status and re.search(self.state_pending_re,
                                             self.job_status)

    def state_isrunning(self):
        assert self.state_running_re, "Misconfigured: define state_running_re"
        return self.job_status and re.search(self.state_running_re,
                                             self.job_status)

    def state_isunknown(self):
        # Blank means "not set" and this function always returns None.
        if self.state_unknown_re:
            return self.job_status and re.search(self.state_unknown_re,
                                                 self.job_status)

    async def state_gethost(self):
