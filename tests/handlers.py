...
def fc_server(httpserver):
    ...
    httpserver.expect_request(
        re.compile(r"/compute/.*/jobs"), method="DELETE"
...
def status_handler(request: Request):
    if request.headers["Authorization"] != "Bearer VALID_ACCESS_TOKEN":   # "Bearer VALID_TOKEN":
        return Response(
            json.dumps({"message": "Bad token; invalid JSON"}),
            status=401,
            content_type="application/json",
    ...
    data = read_json_file(f"responses/{endpoint}.json")
...
def filesystem_handler(request: Request):
    ...
    data = read_json_file(f"responses/{endpoint}{suffix}.json")
